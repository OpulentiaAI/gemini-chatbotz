...
import { PaperclipIcon, PlusIcon, XIcon, ArrowUpIcon, StopCircle } from "lucide-react";
import {
...
export function PromptInputAttachment({
  ...
  return (
    <div
      className={cn("group relative h-14 w-14 border rounded-md overflow-hidden", className)}
      key={data.id}
      {...props}
    >
      {data.mediaType?.startsWith("image/") && data.url ? (
        ...
        <div className="flex size-full items-center justify-center text-muted-foreground bg-muted">
  ...
  const add = useCallback(
    (files: File[] | FileList) => {
      ...
      setItems((prev) => {
        ...
        const newItems = capped.map((file, index) => {
...
export const PromptInputSubmit = ({
  ...
  return (
    <Button
      className={cn("gap-1.5 size-9 rounded-lg", className)}
      size={size}
      type={buttonType}
      variant={variant}
      {...props}
